...
if __name__ == "__main__":
    ...
    if not os.path.exists(stl_filename):
        ...
        # 4. Save Meshes, Centerlines, and Metadata
        output_dir = "output_meshes"
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)
            
        print(f"\nSaving {num_samples} samples to '{output_dir}/'...")
        
        all_meta_list = [] # For the master JSON file

        for sample in dataset:
            s_id = sample['id']
            base_path = os.path.join(output_dir, f"sample_{s_id}")
            
            # --- A. Save the Mesh ---
            sample['mesh'].export(f"{base_path}_mesh.stl")
            
            # --- B. Save the Centerline Points ---
            # Save as .npy (for training) and .csv (for inspection)
            np.save(f"{base_path}_centerline.npy", sample['centerline'])
            np.savetxt(f"{base_path}_centerline.csv", sample['centerline'], 
                       delimiter=",", header="x,y,z", comments="")

            # --- C. Save Individual Metadata (Force/Epicenter) ---
            meta_data = {
                "id": s_id,
                "epicenter_xyz": sample['label']['epicenter_xyz'].tolist(),
                "applied_force": sample['label']['applied_force'].tolist()
            }
            all_meta_list.append(meta_data)

            # Save metadata as .npy for fast ML label loading
            # We stack epicenter and force into one vector: [ex, ey, ez, fx, fy, fz]
            meta_vector = np.concatenate([sample['label']['epicenter_xyz'], 
                                        sample['label']['applied_force']])
            np.save(f"{base_path}_metadata.npy", meta_vector)
            
            # Save metadata as .csv
            np.savetxt(f"{base_path}_metadata.csv", [meta_vector], 
                       delimiter=",", header="epi_x,epi_y,epi_z,force_x,force_y,force_z", comments="")
            
            print(f"  - Saved Sample {s_id}")

        # --- D. Save Master JSON Summary ---
        with open(os.path.join(output_dir, "summary_metadata.json"), "w") as f:
            json.dump(all_meta_list, f, indent=4)
